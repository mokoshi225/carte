@echo off
cd /d "%~dp0"
echo [build-tree] Compiling UIA Tree Viewer...
...
if "%UIA_REF%"=="" ( echo [ERROR] UIAutomationClient.dll not found! & pause & exit /b 1 )
if "%UIA_TYPES_REF%"=="" ( echo [ERROR] UIAutomationTypes.dll not found! & pause & exit /b 1 )
if "%WBASE_REF%"=="" ( echo [ERROR] WindowsBase.dll not found! & pause & exit /b 1 )

%CSC% /target:exe /reference:%UIA_REF% /reference:%UIA_TYPES_REF% /reference:%WBASE_REF% /reference:System.Core.dll /out:uia-tree.exe uia-tree.cs
...
echo [SUCCESS] uia-tree.exe generated.
echo.
echo Usage:
echo   1. Open "カルテ・オーダー入力" window
echo   2. Double-click uia-tree.exe
echo   3. Open uia-tree-output.txt to see the tree
echo.
pause
